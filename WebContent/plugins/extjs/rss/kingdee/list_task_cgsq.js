 ...
 * 采购申请待审列表
 */
CgsqTaskListPanel = Ext.extend(Ext.Panel, {
    layout: 'border',
    border: false,
    closable: true,
  	autoScroll:true,
    initComponent: function() {
		...
		this.store = new Ext.data.JsonStore({
			url:'kingdee.do?cmd=list_task_cgsq',
			root: 'result',
			totalProperty:"rowCount",
			fields:['FCheck','FCloseStatus','hywgb','FInterID','FEntryID','FBillNo','FCancellation','FSourceBillNo','FDate','cpdm','cpmc','cpgg','jldw','fssl','dhrq','ywy','FNote','bz','aqkc','jcsl','FCurCheckLevel'],
	  		remoteSort:true,
        		        	...
        		        	handler: function(){
        	            		Ext.Msg.confirm("确认框","确定要审核吗?",function(btn){ 
        	            			if(btn=='yes'){ 
                				    	...
                				    	var mask = new Ext.LoadMask('list_task_cgsq_grid', {
                				    		msg : '正在处理。。。'
                				    	});
                				    	mask.show();
                				    	
                				    	Ext.Ajax.request({
                				    		url:'kingdee.do?cmd=check_cgsq',
                    	...
                    	colModel: new Ext.ux.grid.LockingColumnModel([
	                        this.sm,
                            {
                                ...
                                renderer:function(value,metadata,record){
	                        		var grid = Ext.getCmp('list_task_cgsq_grid');
	                        		var s = grid.getStore();
        ...
        CgsqTaskListPanel.superclass.initComponent.call(this);
    }
});
