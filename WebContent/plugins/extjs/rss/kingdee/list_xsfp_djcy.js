 ...
 * 销售发票单价与订单单价有差异的列表
 */
SellSaleDiffListPanel = Ext.extend(Ext.Panel, {
    layout: 'border',
    border: false,
    closable: true,
  	autoScroll:true,
    initComponent: function() {
		...
		this.store = new Ext.data.JsonStore({
			url:'kingdee.do?cmd=list_xsfp_djcy',
			root: 'result',
			totalProperty:"rowCount",
			fields:['FInterID','FEntryID','FOrderID','FSaleID','FCheck','Fdate','FBillNo','dwdm','wldw','ywy','cpmc','cpgg','jldw','fssl','wsdj','hsdj','xxs','hsje','cpdm','hxsl','hxje','ddhsdj','ddhsje'],
	  		remoteSort:true,
                        ...
                        tbar:['   ',{
        		        	...
        		        	handler: function(){
        				    	...
        				    	for(var i=0;i<records.length;i++){
        				    		...
        				    		jsonBegin += '{"FInterID":"'+FInterID+'","FEntryID":"'+FEntryID+'"}';
        				    		if(i<records.lenght-1)
        				    			jsonBegin += ',';
        				    	}
        				    	jsonBegin+=jsonEnd;
        				    	
        				    	var mask = new Ext.LoadMask('list_xsfp_djcy_grid', {
        				    		msg : '正在处理。。。'
        				    	});
        				    	mask.show();
        				    	
        				    	Ext.Ajax.request({
        				    		url:'kingdee.do?cmd=update_xsdj',
