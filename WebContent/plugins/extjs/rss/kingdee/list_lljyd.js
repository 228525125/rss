 ...
 * 来料检验单列表
 */
ICQCBillListPanel = Ext.extend(Ext.Panel, {
    layout: 'border',
    border: false,
    closable: true,
  	autoScroll:true,
    initComponent: function() {
		...
		this.store.paramNames.sort="orderBy";
	 	...
	 	this.cgdd = new POOrderSimpleListPanel({id:'lljyd_cgdd_simple',title:'采购订单',iconCls:'pluginIcon'});
	 	this.wgrk = new WgrkSimpleListPanel({id:'lljyd_wgrk_simple',title:'外购入库',iconCls:'pluginIcon'});
	 	this.lljysqd = new POInstockSimpleListPanel({id:'lljyd_lljysqd_simple',title:'来料检验申请单',iconCls:'pluginIcon'});
	 	...
	 	this.bottompanel = new Ext.TabPanel({                    	                        
            ...
            listeners: {
            	'expand': {fn:function(p){  
	            	if(undefined!=p.getActiveTab().djbh&&''!=p.getActiveTab().djbh&&'lljyd_cgdd_simple'==p.getActiveTab().id){
	    				this.cgdd.store.removeAll();                    		
	            		this.cgdd.store.baseParams['query']=this.cgdd.djbh;
	            		this.cgdd.store.load();                            		
	    			}
	            	
	            	if(undefined!=p.getActiveTab().djbh&&''!=p.getActiveTab().djbh&&'lljyd_lljysqd_simple'==p.getActiveTab().id){
	    				this.lljysqd.store.removeAll();                    		
	            		this.lljysqd.store.baseParams['query']=this.lljysqd.djbh;
	            		this.lljysqd.store.load();                            		
	    			}
	            	
	            	if(undefined!=p.getActiveTab().djbh&&''!=p.getActiveTab().djbh&&'lljyd_wgrk_simple'==p.getActiveTab().id){
                ...
                items: [
                    {
                        ...
                        listeners: {
	                		'cellclick': {fn:function(t,rowIndex,columnIndex,e){                    			
	                			...
	                			if(this.bottompanel.isVisible()&&'lljyd_cgdd_simple'==this.bottompanel.getActiveTab().id){                    				
	                				this.cgdd.store.removeAll();                    		
	                        		this.cgdd.store.baseParams['query']=this.cgdd.djbh;
	                        		this.cgdd.store.load();                            		
	                			}
	                			...
	                			if(this.bottompanel.isVisible()&&'lljyd_lljysqd_simple'==this.bottompanel.getActiveTab().id){                    				
	                				this.lljysqd.store.removeAll();                    		
	                        		this.lljysqd.store.baseParams['query']=this.lljysqd.djbh;
	                        		this.lljysqd.store.load();                            		
	                			}
	                			...
	                			if(this.bottompanel.isVisible()&&'lljyd_wgrk_simple'==this.bottompanel.getActiveTab().id){                    				
	                				this.wgrk.store.removeAll();                    		
	                        		this.wgrk.store.baseParams['query']=this.wgrk.djbh;
	                        		this.wgrk.store.load();                            		
	                			}                    			                    			
        ...
        ICQCBillListPanel.superclass.initComponent.call(this);
    }
});
