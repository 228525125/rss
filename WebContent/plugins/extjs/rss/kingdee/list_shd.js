 ...
 * 收货单列表
 */
ShdListPanel = Ext.extend(Ext.Panel, {
    layout: 'border',
    border: false,
    closable: true,
  	autoScroll:true,
    initComponent: function() {
		...
		this.store = new Ext.data.JsonStore({
			url:'kingdee.do?cmd=list_shd',
			root: 'result',
			totalProperty:"rowCount",
			fields:['FCheck','FCloseStatus','hywgb','FInterID','FEntryID','FBillNo','FCancellation','FSourceBillNo','FOrderInterID','FOrderEntryID','FDate','cpdm','cpmc','cpgg','jldw','fssl','wlph','ywy'],
	  		remoteSort:true,
		...
		this.store.paramNames.sort="orderBy";
	 	...
	 	this.cgdd = new POOrderSimpleListPanel({id:'shd_cgdd_simple',title:'采购订单',iconCls:'pluginIcon'});
	 	this.wgrk = new WgrkSimpleListPanel({id:'shd_wgrk_simple',title:'外购入库',iconCls:'pluginIcon'});
	 	...
	 	this.bottompanel = new Ext.TabPanel({                    	                        
            ...
            listeners: {
            	'expand': {fn:function(p){  
	            	if(undefined!=p.getActiveTab().djbh&&''!=p.getActiveTab().djbh&&'shd_cgdd_simple'==p.getActiveTab().id){
	    				this.cgdd.store.removeAll();                    		
	            		this.cgdd.store.baseParams['query']=this.cgdd.djbh;
	            		this.cgdd.store.load();                            		
	    			}
	            	
	            	if(undefined!=p.getActiveTab().djbh&&''!=p.getActiveTab().djbh&&'shd_wgrk_simple'==p.getActiveTab().id){
                ...
                items: [
                    {
                        ...
                        listeners: {
                    		'cellclick': {fn:function(t,rowIndex,columnIndex,e){                    			
                    			...
                    			if(this.bottompanel.isVisible()&&'shd_cgdd_simple'==this.bottompanel.getActiveTab().id){                    				
                    				this.cgdd.store.removeAll();                    		
                            		this.cgdd.store.baseParams['query']=this.cgdd.djbh;
                            		this.cgdd.store.load();                            		
                    			}
                    			...
                    			if(this.bottompanel.isVisible()&&'shd_wgrk_simple'==this.bottompanel.getActiveTab().id){                    				
                    				this.wgrk.store.removeAll();                    		
                            		this.wgrk.store.baseParams['query']=this.wgrk.djbh;
                            		this.wgrk.store.load();                            		
                    			}                    			                    			
        ...
        ShdListPanel.superclass.initComponent.call(this);
    }
});
